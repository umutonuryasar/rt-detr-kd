"""FROZEN TEST FIXTURE — the pre-P-1 HybridEncoder. DO NOT EDIT.

Verbatim copy of ``src/models/encoder.py`` as of commit **023947a**
("Fix KD methodology + training correctness before Phase 2A"), the last commit
before **4d63c97** removed the dead C3 fusion branch (AUDIT.md, fix P-1).

WHY THIS FILE EXISTS
--------------------
`tests/test_audit_regressions.py` proves the P-1 removal changed no output
value: it runs this encoder and the current one on the same input with the
same shared weights and requires the results to be bit-identical, and it
measures the parameter drop (477,824 student / 576,128 teacher) against it.

This was originally loaded with ``git show HEAD:src/models/encoder.py``, which
silently stopped working the moment P-1 was committed — HEAD became the *new*
code, so the tests compared the post-change encoder against itself and reported
a parameter delta of 0. Vendoring the file removes that dependency on git
history entirely and makes the tests work in a shallow clone.

RULES
-----
- **Never edit this file to make a test pass.** It is the historical baseline;
  editing it to match the current encoder destroys the only thing it proves.
- It must stay self-contained. It deliberately does NOT import from
  ``src.models.*`` — if it did, it would track the current code and the bug
  described above would come straight back.
- The only change from the original is this header replacing the module
  docstring. The code below is untouched, including the dead ``c3_fused``
  branch, which is the entire point.
